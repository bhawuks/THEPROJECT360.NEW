...
// ------------------------------------------------------------
// ✅ Manpower Templates (Name-based smart memory)
// Stored at: users/{uid}/manpowerTemplates/{nameKeyDocId}
// ------------------------------------------------------------
const normalizeKey = (s: string) =>
String(s || "").trim().replace(/\s+/g, " ").toLowerCase();

const safeDocId = (nameKey: string) => encodeURIComponent(nameKey).replace(/%/g, "_");

export type ManpowerTemplate = {
nameKey: string;
name: string;
trade?: string;
unit?: string;
...
export const saveManpowerTemplate = async (userId: string, template: ManpowerTemplate) => {
if (!db) {
  console.error("Firestore is not initialized.");
  return;
}
try {
  const nameKey = normalizeKey(template.name || template.nameKey);
  if (!nameKey) return;
  const ref = doc(db, "users", userId, "manpowerTemplates", safeDocId(nameKey));
  await setDoc(ref, { ...template, nameKey, updatedAt: Date.now() }, { merge: true });
} catch (e) {
  console.error("Error saving manpower template: ", e);
}
};

export const getManpowerTemplates = async (userId: string, max = 50): Promise<ManpowerTemplate[]> => {
if (!db) {
  console.error("Firestore is not initialized.");
  return [];
}
try {
  const colRef = collection(db, "users", userId, "manpowerTemplates");
  const qRef = query(colRef, orderBy("updatedAt", "desc"), limit(max));
  const snaps = await getDocs(qRef);
  return snaps.docs.map(d => d.data() as ManpowerTemplate);
} catch (e) {
  console.error("Error getting manpower templates: ", e);
  return [];
}
};

export const getManpowerTemplateByName = async (userId: string, name: string): Promise<ManpowerTemplate | undefined> => {
if (!db) {
  console.error("Firestore is not initialized.");
  return undefined;
}
try {
  ...
  return snap.exists() ? (snap.data() as ManpowerTemplate) : undefined;
} catch (e) {
  console.error("Error getting manpower template: ", e);
  return undefined;
}
};
